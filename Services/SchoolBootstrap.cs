...
public static class SchoolBootstrap
{
    ...
    public static async Task EnsureSchoolBootstrappedAsync(AppDbContext db, Guid schoolId, CancellationToken ct = default)
    {
        ...
        await EnsureSampleTimetableAsync(db, schoolId, ct);
    }

    public static async Task EnsureSampleTimetableAsync(AppDbContext db, Guid schoolId, CancellationToken ct = default)
    {
        if (await db.ClassTimetableSlots.AnyAsync(t => t.SchoolId == schoolId, ct))
            return;
        ...
        var teacherAssignment = await db.TeacherAssignments.AsNoTracking()
            .Where(a => a.SchoolId == schoolId && a.IsActive
                        && a.SchoolClassId == classSection.Value.ClassId
                        && a.SchoolSectionId == classSection.Value.SectionId)
            .FirstOrDefaultAsync(ct);
        ...
        {
            for (var period = 0; period < subjects.Count; period++)
            {
                var subject = subjects[period % subjects.Count];
                var start = new TimeOnly(startHour + period, 0);
                var end = new TimeOnly(startHour + period + 1, 0);
                db.ClassTimetableSlots.Add(new ClassTimetableSlot
                {
                    ...
                    PeriodLabel = $"Period {period + 1}",
                    StartTime = start,
                    EndTime = end,
                    SubjectId = subject.Id,
                    StaffMemberId = teacherAssignment?.StaffMemberId,
